...
class Vivado(Edatool):
        ...
        if api_ver == 0:
            return {'description': "The Vivado backend executes Xilinx Vivado to build systems and program the FPGA",
                    'members': [
                        ...
                        {'name': 'ooc',
                         'type': 'Boolean',
                         'desc': 'Out of Context Synthesis. Allowed values are False (default) and True (out_of_context).'},
                        {'name': 'pr',
                         'type': 'Boolean',
                         'desc': 'Partial Reconfiguration (PR) workflow. Allowed values are False (default) and True (partial_reconfiguration).'},
                        {'name': 'pnr',
                         'type': 'String',
    ...
    def configure_main(self):
        ...
        pr = self.tool_options.get('pr', None)
        ...
        if not pr:
            ...
            # Add pBlock variables
            # Add
            self.render_template('vivado-partial.tcl.j2',
                                 self.name+"_synth.tcl",
                                 synth_template_vars)
        # Write Makefile
